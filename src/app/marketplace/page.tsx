...
const categoryColors: Record<string, string> = {
  ...
  development: 'bg-sky-100 text-sky-700 dark:bg-sky-900/40 dark:text-sky-300',
  ...
  devops: 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300',
}
  ...
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((star) => (
        <svg
          key={star}
          className={`w-4 h-4 ${star <= Math.round(rating) ? 'text-yellow-400' : 'text-gray-200 dark:text-gray-600'}`}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
...
function ListingCard({ listing, installed, onInstall, installing, featured }: {
  ...
  return (
    <div className={`bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-5 hover:shadow-lg dark:hover:shadow-gray-950/30 transition-all flex flex-col ${featured ? 'ring-2 ring-blue-400 dark:ring-blue-500' : ''}`}>
      <div className="flex items-start justify-between gap-2 mb-3">
        <div className="flex gap-2">
          <span className={`inline-block px-2 py-0.5 text-xs font-medium rounded-full ${typeColors[listing.type]}`}>
            {listing.type}
          </span>
          <span className={`inline-block px-2 py-0.5 text-xs font-medium rounded-full ${categoryColors[listing.category] || 'bg-gray-100 text-gray-600'}`}>
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 flex-1 line-clamp-2">{listing.description}</p>
      ...
      <div className="mt-4 flex items-center justify-between">
        <div>
          <p className="text-xs text-gray-400 dark:text-gray-500">by {listing.author}</p>
          <div className="flex items-center gap-3 mt-1">
            <StarRating rating={listing.rating} />
            <span className="text-xs text-gray-400 dark:text-gray-500">{listing.installs.toLocaleString()} installs</span>
          </div>
        </div>
        <button
          onClick={onInstall}
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="text-center mb-12">
        <h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-3">
          Agent <span className="text-blue-600 dark:text-blue-400">Marketplace</span>
        </h1>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-8">
        <input
          ...
          className="w-full sm:w-80 px-4 py-2 rounded-xl border border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-800 dark:text-white shadow-sm"
        />
      </div>
      ...
      <div>
        ...
        {!loaded ? (
          <div className="text-center py-16">
            <svg className="w-8 h-8 animate-spin text-blue-500 mx-auto" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>
            <p className="text-gray-400 mt-3">Loading marketplace...</p>
          </div>
        ) : filtered.length === 0 ? (
          <p className="text-center text-gray-400 dark:text-gray-500 py-16">No listings match your filters.</p>
        ) : (
