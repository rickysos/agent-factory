    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-forge-800 mb-2">
          Provider <span className="text-accent-600">Configuration</span>
        </h1>
        <p className="text-lg text-forge-500">Configure API keys for your AI providers</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-4">
          {initialProviders.map(provider => {
            ...
            return (
              <div key={provider.id} className="bg-forge-50 rounded-md  p-6 border border-forge-200">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h3 className="font-semibold text-forge-800 text-lg">{provider.name}</h3>
                    <span className="text-sm text-forge-400">{provider.docsUrl}</span>
                  </div>
                  {state.status !== 'idle' && (
                    <span className={`px-2 py-0.5 rounded text-xs font-semibold ${
                      state.status === 'success' ? 'bg-accent-500/10 text-accent-600' : 'bg-red-500/50/10 text-red-500'
                    }`}>
                ...
                <div className="flex gap-2">
                  <div className="flex-1 relative">
                    <input
                      ...
                      className="w-full px-4 py-2 pr-16 border border-forge-200 rounded focus:ring-2 focus:ring-accent-500 focus:border-accent-500 outline-none text-forge-800 text-sm font-mono"
                    />
                    <button
                      ...
                      className="absolute right-2 top-1/2 -translate-y-1/2 text-xs text-forge-400 hover:text-forge-600 px-2 py-1"
                    >
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-accent-500 text-forge-950 rounded text-sm font-medium hover:bg-accent-400 transition disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
                  >
        ...
        <div className="lg:col-span-1">
          <div className="bg-forge-50 rounded-md  p-6 border border-forge-200 sticky top-8">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-forge-800">.env Preview</h3>
              <button
                onClick={handleExport}
                disabled={!envPreview}
                className="px-3 py-1.5 bg-accent-500 text-forge-950 rounded text-xs font-medium hover:bg-accent-400 transition disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Export .env
              </button>
            </div>
            <pre className="bg-forge-900 text-accent-400 rounded p-4 text-sm font-mono overflow-x-auto min-h-[200px] whitespace-pre-wrap">
