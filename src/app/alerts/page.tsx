    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-2">Configure alert rules and view triggered alert history.</p>
        </div>
        <button
      ...
      {showForm && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
          <h2 className="text-lg font-bold text-gray-900 mb-5">Create Alert Rule</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-1">Channel Target</label>
              <input
                type="text"
          ...
          <div className="mt-5">
            <button
              ...
              className="px-5 py-2 text-sm font-medium rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition disabled:opacity-40 disabled:cursor-not-allowed"
            >
              Create Rule
            </button>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-8">
        ...
        <div className="divide-y divide-gray-100">
          {rules.map(rule => (
            <div key={rule.id} className="px-6 py-4 flex items-center justify-between hover:bg-gray-50 transition">
              ...
              <div className="flex items-center gap-4">
                ...
                <span className="text-xs text-gray-400 max-w-[160px] truncate">{rule.channelTarget}</span>
                <button
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="divide-y divide-gray-100">
          {alertHistory.map(evt => (
            <div key={evt.id} className="px-6 py-4 hover:bg-gray-50 transition">
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-3">
                  <span className={`px-2 py-0.5 text-xs font-medium rounded ${severityStyles[evt.severity]}`}>
                    {evt.severity}
                  </span>
                  <span className="text-sm font-semibold text-gray-900">{evt.ruleName}</span>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-xs text-gray-400">{new Date(evt.timestamp).toLocaleString()}</span>
                  <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${
                    evt.resolved ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
                  }`}>
                    {evt.resolved ? 'Resolved' : 'Active'}
                  </span>
                </div>
              </div>
              <p className="text-sm text-gray-600 mt-1">{evt.message}</p>
            </div>
          ))}
        </div>
      </div>
